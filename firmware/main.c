...
// This definition is required to calibrate __delay_us() and DelayMs()
	...
	while(1)
	{
		...
		*/
		...
		switch(currentState)
		{
			...
			case state_getAlarm: //is it time to sound the alarm?
			currentState = state_sleep;
			break;
			
			case state_setTime: //set the time!
			IOCIE = 0; // disable CN
			VLED_EN = 1; //enable display supply
			fastClock(); //engage PLL, switch to HFINTOSC
			setTime();
			//setOptions();
			...
			VLED_EN = 0; //kill display supply
			
			TMR0IE = 0;//kill refresh timer

			//ground display row sources
			...
			//ground display column sinks (lowers power consumption)
			MATRIX_TRIS_COLOR_RED = MATRIX_TRIS_COLOR_GRN = MATRIX_TRIS_COLOR_BLU = 0;
			
			//TODO: put serial flash to sleep
			
			//TODO: put accelerometer to sleep?
			
			//kill PLL, switch to MFINTOSC
...
unsigned char init(void)
{
	...
	DelayMs(1); //wait for timer1 oscillator to stabilize
	...
	MATRIX_LAT_COLOR_RED = 0x00; //gpio low (columns on) by default (these are RED column current sinks)
	MATRIX_LAT_COLOR_GRN = 0x00; //gpio low (columns on) by default (these are GREEN column current sinks)
	MATRIX_LAT_COLOR_BLU = 0x00; //gpio low (columns on) by default (these are BLUE column current sinks)
	MATRIX_TRIS_COLOR_RED = 0xFF; //gpio as Hi-Z
	MATRIX_TRIS_COLOR_GRN = 0xFF; //gpio as Hi-Z
	MATRIX_TRIS_COLOR_BLU = 0xFF; //gpio as Hi-Z
	...
	PR2 = 125; //makes for interval of 1ms at 32MHz
...
static void interrupt isr(void) // Here be interrupt function - the name is unimportant.
{
	if((TMR0IE)&&(TMR0IF)) // matrix scan timer interrupt
	{
		...
		MATRIX_TRIS_COLOR_RED = ~rowValueTempR; //set column sinks
		MATRIX_TRIS_COLOR_GRN = ~rowValueTempG; //set column sinks
		MATRIX_TRIS_COLOR_BLU = ~rowValueTempB; //set column sinks
	...
	if((TMR1IE)&&(TMR1IF))//RTC timer interrupt
	{
		...
		if(++seconds > 59)
		{
			...
			//currentState = state_getAlarm; //check for alarm status on the minute
...
void bitmapTest(void)
{
	...
	{
		bitmapPutPixel(currentPixelPosition, 0, MATRIX_PWM_MAX, 0, 0);
	}
	
	for(currentPixelPosition = 0; currentPixelPosition < MATRIX_COLUMNS; currentPixelPosition++)
	{
		bitmapPutPixel(currentPixelPosition, 1, 0, MATRIX_PWM_MAX, 0);
	}
	
	for(currentPixelPosition = 0; currentPixelPosition < MATRIX_COLUMNS; currentPixelPosition++)
	{
		bitmapPutPixel(currentPixelPosition, 2, 0, 0, MATRIX_PWM_MAX);
	}
	
}

void bitmapPutPixel(unsigned char x, unsigned char y, unsigned char r, unsigned char g, unsigned char b)
{
	...
	{		
		...
		if(changesMadeFlag) //apply changes
		{
			...
			for(i=0; i<tempValueTens; i++) //tens column
			{
					if(i < 8)
					{
						bitmapPutPixel((currentFieldSelected<<2), i, 0, 0, MATRIX_PWM_MAX);
						bitmapPutPixel((currentFieldSelected<<2)+1, i, 0, 0, MATRIX_PWM_MAX);
					}
					else
					{
						bitmapPutPixel((currentFieldSelected<<2), 7, MATRIX_PWM_MAX, 0, 0);
						bitmapPutPixel((currentFieldSelected<<2)+1, 7, MATRIX_PWM_MAX, 0, 0);
					}
			}
			...
			for(i=0; i<tempValueUnits; i++) //units column
			{
					if(i < 8)
					{
						bitmapPutPixel((currentFieldSelected<<2)+2, i, 0, 0, MATRIX_PWM_MAX);
						bitmapPutPixel((currentFieldSelected<<2)+3, i, 0, 0, MATRIX_PWM_MAX);
					}
					else
					{
						bitmapPutPixel((currentFieldSelected<<2)+2, 7, MATRIX_PWM_MAX, 0, 0);
						bitmapPutPixel((currentFieldSelected<<2)+3, 7, MATRIX_PWM_MAX, 0, 0);
					}
			}
...
void showTime(void)
{
	...
	x, y, xLast, yLast, tempPixelR, tempPixelG, tempPixelB, pcnt;
	...
	bitmapClear(); //clear display
	
	//turn on display refresh interrupt
	TMR0IE = 1;
	
	//read generic tick timer to process animation frames
	T2CONbits.TMR2ON = 1; //timer2 ON
	
        while(currentState == state_showTime)
	{
                if (pcnt >= 100){
                    shiftUp();
                    generateLine();
...
unsigned char getBattery(void)
{
	...
	//2us needed to charge ADC input. At 1MHz FOSC, FCY = 250kHz.
	//TCY = 4us, so less than one cycle delay is necessary.
	asm("NOP"); //wait acquisition time
